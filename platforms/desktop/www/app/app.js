var remoteCmsURL = "http://ncctest.autoboxdev.com";
			...
			geo.get(function(state) {
				if(state == "on") {
					...
					navigator.geolocation.getCurrentPosition(
						...
						function(e) { 
							alert("Geolocation error, we can't find your location. Please check that GPS services are enabled.");
							initMap("off");
						}, 
						{ 
...
function shareOnFaceBook(description, picture, link, name, tour) {
    ...
    var postUrl = "http://ncc.autoboxdev.com/social/fb/fbPost.php"
...
$(document).delegate("#wp-share-tw", "click", function(e) {
	...
	var img, share, title, twUrl, tweet, stringLen,
	...
	tweet = share + "... " + tweet;
	twUrl = remoteCmsURL+"/social/twitter/index.html?content=" + encodeURIComponent(tweet);
...
function renderTourSets() {
  ...
  //$('.tour-times .radio-tourtime').filter('[value="1"]').checkboxradio().first().attr('checked',true).checkboxradio("refresh");
...
function renderTours(){
  ...
  /*
  var img = $(".tourset-landing-main-img img").attr("large");
  var title = $(".tourset-landing-title").text();
  var precis = $(".tourset-landing-precis").html();
  */
...
function showTopics( urlObj, options ) {
	...
	//if() {}
	//currentTopicsType.set(topicType);
			...
			var markup = '<h3 class="topic-detail-title">' + topicsData[tIndex].Term + '</h3><ul data-role="listview" data-inset="false">';
	  		...
	  		store.waypoints.all(function(data) {
	  			$.each(data, function(j, record) {
					...
					if($.inArray(tid, tourTopics) > -1) {
						markup += '<li><a href="#waypoint" onclick="javascript:initWaypointDetail(' + '\'' + record.key + '\'' + '); return false;"><img src="' + record.value.Main_image_thumb[0] + '" alt="" /><h3>' + record.value.Title + '</h3><p>' + record.value.Topics + '</p></a></li>';
...
function renderTour(data) {	
	...
	// NID was global - perhaps by accident? RE
	// var title, img, imgurl, body, nid;
...
function insertWP(item) {
	...
	var mainImage = (typeof item.value.Main_image_thumb === 'undefined') ? '<img src="" />' : '<img src="' + item.value.Main_image_thumb[0] + '" />';
  	var listItem = '<li><a href="#waypoint" data-waypoint-detail-id="' + item.value.Tnid + '">' + mainImage +' <h3>' + title + '</h3><p>' + topics + '</p></a></li>';
...
function showWayPointDetail() {
	...
	//if the visitor is 250m or less from WP bieng viewed, tag via analytics
	geo.get(function(state) {
		if(state == "on") {
			var geo = geoFence(item.value.Coordinates) ? '#geo' : '';
...
var createStaticMap = function() {
	...
	//console.log('http://maps.google.com/maps/api/staticmap?center='+ getGeoAverage(coordsList) + '&zoom=15&size=320x206&scale=2&maptype=roadmap' + mkrStr + '&sensor=false&key=AIzaSyAlyOD4eG246HLkKW-YiRCdDMdfA5z8VSU');
	
	return 'http://maps.google.com/maps/api/staticmap?center='+ getGeoAverage(coordsList) + '&zoom=15&size=320x206&scale=2&maptype=roadmap' + mkrStr + '&sensor=false&key=AIzaSyAlyOD4eG246HLkKW-YiRCdDMdfA5z8VSU';
}
...
function showWpAudio( urlObj, options ) {
	...
	store.waypoints.get(wpId, function(item) {
      ...
      // Used to test Android
      //audio[i] =  "http://tonycuffe.com/mp3/tailtoddle_lo.mp3";
        ...
        //			markup += '<li><a href="' + audio[i] + '" rel="external"><h3 class="ui-li-heading">' + clipTitle + '</h3><p class="ui-li-desc">' + caption + '</p><img src="css/images/icon-audio-@2x.png" /></a></li>';
...
//		  //console.log(markup)
	...
	//$header.find( "h1" ).attr( "href", backLink );	
